<div class="d-flex flex-column flex-md-row px-4 py-2 mt-4 text-white bg-indigo rounded shadow">
  ...
  <div class="d-flex align-items-center me-md-auto">
    ...
    <h1 class="h5 pt-2">Pencarian Data IP</h1>
  </div>
  <!-- breadcrumbs -->
...
<div class="px-4 py-3 mt-3 bg-body rounded shadow">
  <div class="d-flex flex-column flex-md-row">
    ...
    <a href="?halaman=data" class="btn btn-primary mb-3 mb-md-0">
      <i class="far fa-arrow-alt-circle-left me-1"></i> Data IP
    </a>
    <!-- form pencarian -->
    <form action="?halaman=pencarian" method="post" class="form-search needs-validation ms-md-auto" novalidate>
      <input type="text" name="kata_kunci" class="form-control" placeholder="Cari IP.." autocomplete="off">
    </form>
  </div>
</div>
...
<div class="p-4 mt-3 bg-body rounded shadow">
  ...
  ?>
    ...
    $query = mysqli_query($mysqli, "SELECT id_member, tanggal_upload, jenis_pilot, ip, foto_profil FROM tbl_member 
                                    WHERE id_member LIKE '%$kata_kunci%' OR ip LIKE '%$kata_kunci%'
      ...
      while ($data = mysqli_fetch_assoc($query)) { ?>
        ...
        <div class="d-flex flex-column flex-md-row align-items-center align-items-md-left">
          ...
          <div class="text-center text-md-start pb-3 pb-md-0">
            <h6 class="text-uppercase mb-1"><span class="text-primary"><?= $data['id_member']; ?></span> - <?= $data['ip']; ?></h6>
            <div>
              User <span class="text-warning"><?= $data['jenis_pilot']; ?></span>.
              TanggalUpload <span class="text-warning"><?= tanggal_indo($data['tanggal_upload']); ?></span>.
            </div>
          </div>
          <div class="ms-md-auto">
            ...
            <!-- button hapus data -->
            <a href="proses_hapus.php?id=<?= $data['id_member']; ?>" onclick="return confirm('Anda yakin ingin menghapus data member <?= $data['ip']; ?>?')" class="btn btn-danger btn-sm mb-2 mb-md-0">
